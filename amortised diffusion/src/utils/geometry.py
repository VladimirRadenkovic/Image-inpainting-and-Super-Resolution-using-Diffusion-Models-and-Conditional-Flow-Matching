...
def differentiable_kabsch(
    p1: torch.Tensor, p2: torch.Tensor, max_iter: int = 10000
    ...
    assert not torch.isnan(cov).any(), "Covariance matrix is NaN."
    ...
    _has_degenerate_singular_values = (
        lambda S: (S**2 - (S**2).view(num_coords, 1) + torch.eye(num_coords, device=cov.device)).abs().min() < 1e-2
    )
    num_it = 0
    while _has_zero_singular_value(S) or _has_degenerate_singular_values(S):
        ...
        # WARNING! This is the previous implementation by Kieran, which in not differentiable in-place operator
        # cov += (torch.rand(3, device=cov.device) * noise_scale).diag()
        # U, S, Vt = torch.linalg.svd(
        #     cov, full_matrices=True
        # )  # (num_coords, num_coords), (num_coords), (num_coords, num_coords)
...
if __name__ == "__main__":
    ...
    num_points = 100
    p1 = torch.rand(num_points, 3)

    # Apply random rototranslation
    R = random_rotation_matrix(validate=True)
    t = torch.rand(3)
