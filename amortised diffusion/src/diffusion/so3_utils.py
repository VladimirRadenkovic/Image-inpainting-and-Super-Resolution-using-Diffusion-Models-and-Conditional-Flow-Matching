...
def hat(v):
    """
    Computes the hat map/skew-symmetric operator of a given tensor `v`.
    Maps from the vector space R^3 to the Lie algebra so(3).

    Args:
        v (torch.Tensor): The input tensor in R^3. It should be of shape `[..., 3]`

    Returns:
        torch.Tensor: The output tensor  in so(3) after applying the hat operator. 
        The output tensor will be of shape `[..., 3, 3]`.
    """
    if v.shape[-1] != 3:
...
def vee(A):
    """
    Computes the vee map of a given tensor `A`.
    Maps from the Lie algebra so(3) to the vector space R^3.

    Args:
        A (torch.Tensor): The input tensor in so(3). It should be of shape `[..., 3, 3]`

    Returns:
        torch.Tensor: The output tensor in R^3 after applying the vee operator.
        The output tensor will be of shape `[..., 3]`.
    """
    if not torch.allclose(A, -A.transpose(-1, -2)):
        raise ValueError("Input A must be skew symmetric")
    vee_A = torch.stack([-A[..., 1, 2], A[..., 0, 2], -A[..., 0, 1]], dim=-1)
    return vee_A

def Log(R):
    """
    Computes the logarithmic map from SO(3) to R^3 (i.e. rotation vector).

    Args:
        R (torch.Tensor): The input tensor in SO(3). It should be of shape `[..., 3, 3]`

    Returns:
        torch.Tensor: The output tensor in R^3 after applying the logarithmic map.
        The output tensor will be of shape `[..., 3]`.
    """
...
def Omega(R, eps=1e-4):
    """
    Computes the angle of rotation SO(3) to R^+ via θ = arccos((trace(R) - 1) / 2), 
    this is the norm in our chosen orthonormal basis.
    
    Args:
        R (torch.Tensor): The input tensor. It should be of shape `[..., 3, 3]`
        eps (float, optional): A small number to prevent instability of arccos when provided with -1 or 1 as input. Defaults to 1e-4.
    
    Returns:
        torch.Tensor: The output tensor after applying the norm. The output tensor will be of shape `[..., 1]`."""
...
def expmap(R0, tangent):
    """
    Computes the exponential map from tangent space at R0 to SO(3).

    Args:
        R0 (torch.Tensor): The input tensor. It should be of shape `[..., 3, 3]`
        tangent (torch.Tensor): The input tensor. It should be of shape `[..., 3]`

    Returns:
        torch.Tensor: The output tensor after applying the exponential map. The output tensor will be of shape `[..., 3, 3]`.
    """
...
def sample_uniform(N, M=1000):
    """
    Samples from uniform distribution on SO(3) via inverse transform sampling.

    Args:
        N (int): Number of samples to generate from uniform distribution on SO(3).
        M (int, optional): Number of discretisation points to generate CDF. Defaults to 1000.

    Returns:
        torch.Tensor: The output tensor after sampling from uniform distribution on SO(3).
        The output tensor will be of shape `[..., 3, 3]`.
    """
