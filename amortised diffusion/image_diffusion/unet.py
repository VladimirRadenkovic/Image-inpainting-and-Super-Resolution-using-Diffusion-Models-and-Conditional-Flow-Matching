...
def load_matching_weights(model, pretrained_state_dict):
    """
    Load weights from pretrained_state_dict into model that have matching shapes.
    ...
    for name, param in model_state_dict.items():
        if name in pretrained_state_dict and param.shape == pretrained_state_dict[name].shape:
            ...
            print("Init random", name)
            # If the parameter doesn't exist in the pre-trained state dict or shapes don't match
            # randomly initialize
...
def create_model(
    *,
    ...
    try:
        ...
        # if checkpoint contains ema model, load that one.
        if "ema" in state.keys():
            state = {
                k[len("ema_model."):]: v for k, v in state['ema'].items() if "ema_model" in str(k)
            }
        ...
        try:
            print("Trying to load matching weights only.")
            model = load_matching_weights(model, state)
        except:
            print("Could not load matching parameters. Initializing randomly.")
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps):
        ...
        :param x: an [N x Cin x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        :return: an [N x Cout x ...] Tensor of outputs.
        """
        hs = []
