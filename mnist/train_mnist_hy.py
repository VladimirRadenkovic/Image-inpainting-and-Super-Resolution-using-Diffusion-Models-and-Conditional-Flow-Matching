...
from utils_mnist_hy import ema, generate_samples, infiniteloop, generate_samples_eval
...
def train(argv):
    ...
    def plots(step, **kwargs):
        ...
        for i in range(len(x1)):
            ...
            cond = F.interpolate(cond_image[i,:].unsqueeze(0), (64, 64), mode="bilinear")
            plot_condition(test_images[i], cond, axes[i, 1])
            axes[i, 0].axis('off'); axes[i, 1].axis('off')
        fig.tight_layout()
        writer.write_figures(step, dict(condition=fig))
   ...
   #optimizer = torch.optim.Adam(model.parameters())
    #optim = torch.optim.Adam(net_model.parameters())
    #learning_rate = 1e-5
    learning_rate = 2e-4  # Example of a smaller learning rate
