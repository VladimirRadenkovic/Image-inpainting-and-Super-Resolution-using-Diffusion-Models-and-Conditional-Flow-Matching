...
from utils_mnist2 import ema, generate_samples, infiniteloop, generate_samples_eval
...
import numpy as np
def get_random_patch(image_size=28, patch_size=20):
...
def get_loss_weight_mask(image_size, patch_size, h, w):
    """
    Create a weight mask with higher values for the patch area.
    """
    mask = torch.ones(image_size, image_size)
    mask[h:h+patch_size, w:w+patch_size] = 10  # Assign higher weight for the patch
    return mask

def modified_sample(images, pad_value=2, patch_size=14):
    image_size = images.shape[-1]
    pad_value = -2
    patch_size = 20
    h, w = get_random_patch(image_size, patch_size)
    ...
    slice_ = np.s_[:, :, h : h + patch_size, w : w + patch_size]
    condition[slice_] = pad_value
    weight_mask = get_loss_weight_mask(image_size, patch_size, h, w)
    return condition, weight_mask
...
def fit(net_model, ema_model, flow_model, num_epochs, train_dataloader, test_dataset, optimizer, device, savedir, callbacks,
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            ...
            con, weight_mask = modified_sample(x1)
            con = con.to(device)
            ...
            loss = torch.mean(weight_mask * ((vt - ut) ** 2))
            loss.backward()
...
def train(argv):
    ...
    """
    # Data
    train_dataset = get_dataset('flowers')("./data", train=True)
    ...
    test_dataset = get_dataset('flowers')("./data", train=False)
    ...
    def plots(step, **kwargs):
        ...
        for i in range(len(x1)):
            ...
            #cond_image[i] = F.interpolate(cond_image[i], (28, 28), mode="bilinear")
    ...
    callbacks = [
            ...
            PeriodicCallback(every_steps=total_steps//10, callback_fn=plots),
        ]
   ...
   #model = net_model
    ...
    learning_rate = 1e-4  # Example of a smaller learning rate
    optim = torch.optim.Adam(net_model.parameters(), lr=learning_rate)
